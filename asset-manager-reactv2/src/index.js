...
var AssetManager = React.createClass({
        ...
        render: function() {
          return (
            <div className="assetManager" align="center">
              <h1 data-test-id="header" className="jumbotron"> Asset Manager </h1>

              <p> What does your risk profile look like? </p>
              <button data-test-id="riskButton" className="btn btn-primary" type="button">
                Risk <span data-test-id="riskValue" className="badge">{this.state.risk}</span>
              </button>
              <input type="range" name='risk' min='0' max='10' onChange={this.handleChange} onInput={this.handleChange}/>
              <output for="risk" onforminput="value = risk.valueAsNumber;"></output>
              <div data-test-id="pieChartContainer">
              <PieChart
                data={[
                ...
                title="Pie Chart"/>
              <span data-test-id="bonds"> Bonds: {this.props.possibilities[this.state.risk][0]}% </span>
              <span data-test-id="stocks"> Stocks: {this.props.possibilities[this.state.risk][1]}% </span>
              <span data-test-id="etfs"> ETFs: {this.props.possibilities[this.state.risk][2]}% </span>
              <span data-test-id="realstate"> Real Estate: {this.props.possibilities[this.state.risk][3]}% </span>
              <span data-test-id="cash"> Cash: {this.props.possibilities[this.state.risk][4]}% </span>
              </div>
